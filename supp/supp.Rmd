...
# **Appendix S5**: More on Geary's Test and What to do ** UNDER CONSTRUCTION **

### Introduction
Many ecological data sets will have data that do not meet normality assumptions (e.g., abundance data). In other words, the variables one wishes to meta-analyse will often have extreme values which can create problems when calculating $CV^2$ because it will lead to extreme between study $CV^2$. This will create problems when applying our missing data methods. Some adjustments are necessary to the calculation of $CV^2$ under these conditions. 

More formally, we recommend that authors explicitly test whether an effect size meets normality assumptions (which in fact is good practice in any case). This can be achieved using Geary's test (proposed by @Lajeunesse2015). More specifically, authors should test whether each treatment group (control and treatment) pass the test, and if either fails, the data fails the test. We then recommend that $CV^2$ be calculated with only those effect sizes that pass Geary's test. 

### Adjustment to `cv_avg`

The `cv_avg` function has built in capabilities to do this through the use of the `geary` argument. By default, this is set to TRUE. Authors can provide their data, it will conduct Geary's tests and calculate the within and between study $CV^2$ using only the effect sizes passing the test. It will also indicate which rows within the data fail the test so this information can be formally reported in the meta-analysis. 
...
#' @param sub_b A logical indicating whether the between study CV^2 (b_CV2) should be appended to the data only ('TRUE') or whether both within study CV^2 (w_CV2), mean sample size (n_mean) and between study CV^2 (b_CV2) should all be appended to the data only ('FALSE')
#' @param cv2 A logical indicating whether one should take the weighted average of CV2 or the weighted average of CV followed by squaring this average. Default to FALSE.
#' @param geary A logical indicator specifying whether the CV (or CV2) should be calculated only with the data passing Geary's test. Defaults to true to avoid extremely large CV's when data fails to meet assumptions of normality. 
#' @example \dontrun{
#' # test data for cv_avg function
#' library(tidyverse)
#' set.seed(76)
...
#' # # Now generate some missing data
#' t2 <- gen.miss(test_dat, "sd1", "sd2", 6)
#' t2_cv2 <- cv_avg(x = x1, sd = sd1, n = n1, group = stdy, data =  t2, sub_b = TRUE, cv2 = TRUE, geary = FALSE)
#' t2_cv2 <- cv_avg(x2, sd2, n2, group = stdy, label = "2", data =  t2_cv2, sub_b = FALSE, geary = FALSE)
#' # Check calculations are correct. All match what is expected
#' test <- t2_cv2 %>%  filter(stdy == "1")
#' # Within
#' mean(test$n1) # Matches 14.75
#' mean(test$n2) # Matches 14
#' # CV^2
...
cv_avg <- function(x, sd, n, group, data, label = NULL, sub_b = TRUE, cv2=FALSE, geary = TRUE){
  ...
  if(geary){
    data <- data %>% 
            mutate(geary_num = geary({{x}}, {{sd}}, {{n}}),
                  geary_test = ifelse(geary_num >= 3, "pass", "fail")) %>% 
            rename_with(.cols = starts_with("geary"), ~ paste0(.x, "_", label)) %>% 
            mutate(tmp = ifelse(geary({{x}}, {{sd}}, {{n}}) >= 3, "pass", "fail"))
  }
  ...
  b_grp_cv_data <- data                                   %>%
    dplyr::group_by({{group}})                            %>%
    dplyr::mutate(   w_CV2 = ifelse(tmp == "pass", 
                                     weighted_CV({{sd}}, {{x}}, {{n}}, cv2=cv2), NA),
                     n_mean = mean({{n}}, na.rm = TRUE), )   %>%
    dplyr::ungroup(.)                                     %>%
    dplyr::mutate(b_CV2 = weighted.mean(w_CV2, n_mean, na.rm = TRUE), .keep = "used")
  } else{
  ...
  # Append these calculated columns back to the original data and return the full dataset.
  if(sub_b){
    b_grp_cv_data <- b_grp_cv_data %>% dplyr::select(grep("b_", names(b_grp_cv_data))) %>% dplyr::select(-grep("tmp", names(b_grp_cv_data)))
    dat_new <- cbind(data, b_grp_cv_data)
  } else {
    b_grp_cv_data <- b_grp_cv_data %>% dplyr::select(-grep("tmp", names(b_grp_cv_data)))
    dat_new <- cbind(data, b_grp_cv_data)
  }
...
# You also need the helper functions
...
#' @title geary
#' @description Function to calculate Geary's "number"
#' @param mean Mean of an group 
#' @param sd Standard deviation of group
#' @param n The sample size of group
  geary <- function(mean, sd, n){
    (1 / (sd / mean)) * ((4*n)^(3/2) / (1 + 4*n))
  }
...
### Step 1: Geary's Test and calculate `cv_avg` with only data passing the test

Using our function we can now calculate the between-study CV using the data that pass Geary's test while also summarise how much data fail. 
 
```{r, echo = TRUE, eval = FALSE, results='hide', tidy=FALSE, tidy.opts=list(width.cutoff=80), class.source='klippy'}

data1 <- cv_avg(x = Control_mean, sd = Control_standard_deviation, n = Control_sample_size, data = data1, geary = TRUE, label = 'ctrl', sub_b = TRUE)
data1 <- cv_avg(x = Experimental_mean, sd = Experimental_standard_deviation, n = Experimental_sample_size, data = data1, geary = TRUE, label = 'trt')

data1 <- cv_avg(x = Control_mean, sd = Control_standard_deviation, n = Control_sample_size, data = data1, geary = FALSE, label = 'ctrl', sub_b = TRUE)
data1 <- cv_avg(x = Experimental_mean, sd = Experimental_standard_deviation, n = Experimental_sample_size, data = data1, geary = FALSE, label = 'trt')
```

 
### Propagate the CV to the entire dataset

### Step 2:

+ Only meta-analyse effects that pass Geary's test. 
+ We want to meta-analyse CV in the control and treatment groups, then we use this CV for replacement of missing data
+ 
