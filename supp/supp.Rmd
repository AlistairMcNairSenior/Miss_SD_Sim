...
Importantly, this tutorial is self-contained. The code will download all the necessary files needed to run the examples directly to your R console. This includes data from @bird2019 and their phylogenetic tree, and data from @mcdonald2019. Here we provide only the relevant code to demonstrate the methods, but more detailed code for all examples can all be found on our [GitHub](https://github.com/AlistairMcNairSenior/Miss_SD_Sim) repository. Similarly, if users experience any issues with downloading the files needed to run any code they are hosted on [OSF](https://osf.io/h9x6w/).
...
# **Appendix S2**: Worked Example 1 - Dealing with missing SD data to explore the fitness impacts of competition between herbivorous insects
...
source("https://osf.io/sqr4w/download")
```
...
# Download the data file from OSF and do some cleaning
data1 <- read.csv("https://osf.io/xw7yu/download")
...
# Download the phylogeny from OSF
 tree <- read.tree("https://osf.io/8cme5/download")
...
# **Appendix S3**: Worked Example 2 - Dealing with missing SD data when assessing strategic-rest grazing (SRG) regimes on both ungrazed and constantly grazed (CG) systems
...
# Load the data file and tree file
data2 <- read.csv("https://osf.io/at6pn/download")
