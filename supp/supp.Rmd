...
$$
\ln{\text{RR}}_{i} = \beta_{0} + \sum^{N_{mod}}_{k = 1}\beta_kx_k + a_h + q_h + s_j + u_i + m_i,
$$ $$
a_{h} \sim \mathcal{N}(0, \sigma_a^2\textbf{A}), q_{h} \sim \mathcal{N}(0, \sigma_q^2), m_i \sim \mathcal{N}(0, \mathbf{M}^*) 
$$

where $x_{k}$ is the kth moderator's value and $β_{k}$ is the regression coefficient of the kth moderator (h = 1, 2, ..., $N_{mod}$, the number of moderators), $a_{h}$ is the phylogenetic effect for the hth species, considered multivariate normally distributed with a covariance of $\sigma_{a}^2$ "A" [A is a correlation matrix derived from a phylogeny; see @Hadfield2010]; $q_{h}$ is the non-phylogenetic effect for the hth species, distributed with the variance of $\sigma_{q}^2$ ($h$ = 1, 2, ... $N_{species}$, the number of species, which is different from Nstudy \> Neffect-size); and the other notations are the same as above. $\bf{M}$\* is a variance-covariance matrix of the sampling variance, which may result, say, when effect sizes share a common-control [e.g., @Noble2017]; for example, when we have 5 cases from 3 studies, $\bf{M}$\* can be written as:

$$
\mathbf{M}^* = 
     \begin{bmatrix}
    v_1 & \rho \sqrt{v_{1} v_2} & 0 & 0 & 0 \\
    \rho \sqrt{v_{2} v_{1}} & v_2  & 0 & 0 & 0 \\
    0 & 0 &  v_3 & \rho \sqrt{v_3 v_4} & 0 \\
    0 & 0 & \rho \sqrt{v_4 v_3}  & v_4 & 0 \\
    0 & 0 &  0 & 0  & v_5
\end{bmatrix}
$$
...
Constructing $\bf{M}$\* may be as challenging as just doing MI for many ecologists, because the actual value of $\rho$ is often unknown [@Noble2017]. Fortunately, @Hedges2010 derived the robust variance estimator (RVE), which bypasses these challenges by estimating $\rho$ from the data. By using RVE we need only construct $\bf{M}$\*, rather than $\\$\* [see also @Tipton2013]. We show an implementation of this procedure in Appendix S3, using the `clubSandwich` package [@Pustejovsky2021b], which implements the RVE method with a multilevel meta-analysis in `metafor` [cf. @Pustejovsky2021].
...
We will now calculate a new sampling variance using Equation 7, that makes use of the weighted between study $CV^2$ for all observed effect sizes.
...
In Appendix S1 we mention that robust variance estimators can also be used to control for dependency among effect sizes.  We'll demonstrate how missing data methods can be combined with RVE's when using worked example 2 for Method 1A. Example 2 contains a few effect sizes that share a common control group. This creates dependency among their sampling variances that we should control for in our models. However, other sources of non-independence may also be present that we are unaware of (e.g., shared traits; See @Noble2017). Cluster-robust methods are known to do an excellent job at correcting standard errors when such dependeny exists [see @Song2021; @Nakagawa2021; @Hedges2010; @Tipton2013]. 
...
# Using our V matrix, we can re-fit our multi-level meta-analytic model
       MLMA1A_adj <- rma.mv(yi = lnrr_laj, V = V, random=list(~1|Study, ~1|ES_ID), data=data2)
       
# Now we can apply cluster-robust inferences. We have little dependency in these data so we don't expect many changes
    MLMA1A_robust <- robust(MLMA1A_adj, cluster = data2$Study, adjust = TRUE)
    MLMA1A_robust
```
