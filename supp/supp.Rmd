...
```{r echo = TRUE, results='hide', tidy=TRUE, tidy.opts=list(width.cutoff=80), class.source='klippy'}
#' @title cv_avg
...
cv_avg <- function(x, sd, n, group, data, label = NULL, sub_b = TRUE){
  ...
  b_grp_cv_data <- data                                             %>%
    ...
    dplyr::mutate(w_CV2 = weighted.mean(na_if(({{x}} / {{sd}})^2, Inf), {{n}},na.rm = TRUE),n_mean = mean({{n}}, na.rm = TRUE))   %>%
    dplyr::ungroup(.)                                     %>%
