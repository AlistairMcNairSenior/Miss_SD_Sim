...
cv_avg <- function(x, sd, n, group, data, label = NULL, sub_b = TRUE, cv2=FALSE, geary = TRUE){
  ...
  if(geary){
    data <- data %>% 
            ...
            rename(paste0("geary_num", "_", label) = "geary_num",  #### NEED TO FIX RENAME
                   paste0("geary_test", "_", label) = "geary_test")
  }
  ...
  b_grp_cv_data <- data                                   %>%
    dplyr::group_by({{group}})                            %>%
    dplyr::mutate(   w_CV2 = ifelse(geary_test == "pass", 
...
data1 <- cv_avg(x = Control_mean, sd = Control_standard_deviation, n = Control_sample_size, data = data1, geary = TRUE, label = 'ctrl')
