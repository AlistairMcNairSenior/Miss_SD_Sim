...
for(p in 1:nrow(parameters)){
	...
	for(n in 1:n_reps){
		...
		while(sum(is.na(template_p[n,])) > 0){
			...
			model_w<-try(rma.mv(yi = yi, V = vi, random=list(~1|Study, ~1|Effect), data=data_n), silent=T)
			...
			if(class(model_w)[1] == "rma.mv"){
				...
				template_p$whole_Tau2[n]<-sum(model_w$sigma2[c(1:2)])
				template_p$whole_ICC[n]<-model_w$sigma2[1] / sum(model_w$sigma2[c(1:2)])
			}
			...
			# Fit the model
			model1.1<-try(rma.mv(yi = yi, V = vi, random=list(~1|Study, ~1|Effect), data=data_n), silent=T)
			
			# Save the results if the model fitted
			if(class(model1.1)[1] == "rma.mv"){
				template_p$m1.1_ests[n]<-model1.1$b
				template_p$m1.1_SE[n]<-model1.1$se
				template_p$m1.1_Tau2[n]<-sum(model1.1$sigma2[c(1:2)])
				template_p$m1.1_ICC[n]<-model1.1$sigma2[1] / sum(model1.1$sigma2[c(1:2)])
			}
			...
			# Fit the model
			model1.2<-try(rma.mv(yi = yi_2, V = vi_2, random=list(~1|Study, ~1|Effect), data=data_n), silent=T)
			
			# Save the results if the model fitted
			if(class(model1.2)[1] == "rma.mv"){
				template_p$m1.2_ests[n]<-model1.2$b
				template_p$m1.2_SE[n]<-model1.2$se
				template_p$m1.2_Tau2[n]<-sum(model1.2$sigma2[c(1:2)])
				template_p$m1.2_ICC[n]<-model1.2$sigma2[1] / sum(model1.2$sigma2[c(1:2)])
			}
			...
			model2<-try(rma.mv(yi = yi, V = 0, random=list(~1|Study, ~1|Effect, ~1|ID2), data=data_n, R=list(ID2=Vf), Rscale=F), silent=T)
			
			# Save the results if the model fitted
			if(class(model2)[1] == "rma.mv"){
				template_p$m2_ests[n]<-model2$b
				template_p$m2_SE[n]<-model2$se
				template_p$m2_Tau2[n]<-sum(model2$sigma2[c(1:2)])
				template_p$m2_ICC[n]<-model2$sigma2[1] / sum(model2$sigma2[c(1:2)])
			}
			...
			# Try fit the model
			model3<-try(rma.mv(yi = yi, V = vi, random=list(~1|Study, ~1|Effect, ~1|ID2), data=data_n, R=list(ID2=Vf), Rscale=F), silent=T)
			
			# Save the results if the model fitted
			if(class(model3)[1] == "rma.mv"){
				template_p$m3_ests[n]<-model3$b
				template_p$m3_SE[n]<-model3$se
				template_p$m3_Tau2[n]<-sum(model3$sigma2[c(1:2)])
				template_p$m3_ICC[n]<-model3$sigma2[1] / sum(model3$sigma2[c(1:2)])
			}
...
save(results, file=paste0("Simulation_MLMA_", index, ".Rdata"))
save(parameters, file="Parameters_MLMA.Rdata")
